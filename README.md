...
This fork keeps `get-engram/engram` as its upstream baseline and adds a thin local-only runtime for personal use. The local runtime reuses the upstream MCP tools, services, chunking, search logic, and database migrations while adapting D1 to Node 24 SQLite, Workers AI to a localhost LM Studio embedding endpoint, Vectorize to local derived vector storage, and R2 message bodies to inline SQLite. No Engram account, API key, Cloudflare service, or hosted vector database is required for this mode. An optional read-only ABPT adapter exposes ABPT-owned local ChatGPT evidence through the same MCP surface without copying it into Engram or invoking ABPT's live ChatGPT search lane. See [Self-hosting](docs/self-hosting.md#local-personal-mode-acidicsoil-fork).
