...
This fork keeps `get-engram/engram` as its upstream baseline and adds a thin local-only runtime for personal use. The local runtime reuses the upstream MCP tools, services, chunking, search logic, and database migrations while adapting D1 to Node 24 SQLite, Vectorize to local derived vector storage, and R2 message bodies to inline SQLite. Semantic embeddings run in-process through `node-llama-cpp`; the pinned EmbeddingGemma GGUF model is acquired automatically on first use and cached under `~/.cache/engram/models` (or `$XDG_CACHE_HOME/engram/models`). No LM Studio, Ollama, separate model daemon, Engram account, API key, Cloudflare service, or hosted vector database is required. An optional read-only ABPT adapter exposes ABPT-owned local ChatGPT evidence through the same MCP surface without copying it into Engram or invoking ABPT's live ChatGPT search lane. See [Self-hosting](docs/self-hosting.md#local-personal-mode-acidicsoil-fork).
